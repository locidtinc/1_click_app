...
class QrBarcodePrd extends StatefulWidget {
  const QrBarcodePrd({
    super.key,
  ...
  @override
  State<QrBarcodePrd> createState() => _QrBarcodePrdState();
}

class _QrBarcodePrdState extends State<QrBarcodePrd> {
  ...
  void _onQRViewCreated(QRViewController controller) {
    ...
    controller.scannedDataStream.listen((scanData) {
      ...
      widget.productCreateCubit.barCodeChange(scanData.code ?? '');
      ...
      timer ??= Timer(const Duration(milliseconds: 100), () async {
        await widget.productCreateCubit.getProductByScan(scanData.code ?? '');
        timer = null;
      });
    });
  }
